...
async function main() {
   ...
   * GLTF
   */
  let glTF = await LoadJSONUsingPromise("../resource/gltf/cameras.gltf");
  ...
  glTF_TREE.RAW_nodesData.forEach((node) => {
    if (node.hasOwnProperty("camera")) {
      ...
      let camera = new CAMERAS.Camera(gl, node.props.type, node);
      camera.node = node;
      cameras.push(camera);
    }
  });

  let cameraDefault = new CAMERAS.Camera(gl); //constructor(gl,cameraType,{props})

  gl.camera = cameras[gui.cameraindex];
  let camera = cameras[gui.cameraindex];
  gui.camera = camera;
  //=============================================================
  /**
   * HELPERS
