...
export class gltfScene{
    ...
    if ("byteStride" in bufferViews_DATA) {
      byteStride = bufferViews_DATA.byteStride;
    }
...
function decoder64forWebGl(str, type, byteOffset, byteLength, count,componentType,byteStride=0) {
  ...
  // Случай когда Uint16Array 2 байта * VEC4 = 8 байт, но byteStride = 16 байт
  // Получайться  |X - D| |X - D| |X - D| |X - D| где |X - D| D = Uint16Array , X - пустые два байта для выравнивания
  if (byteStride != 0 && componentType == 5123) {
    dView = new DataView(new ArrayBuffer(T.BYTES_PER_ELEMENT * 2));
    fAry = new T(count * elementOfType * 2);
    lenBelement = T.BYTES_PER_ELEMENT * 2;
  }

  byteStride = byteStride == 0 ? elementOfType * lenBelement : byteStride;
  let ELEMENT = 0; // Счетчик элемента в массиве
  for (let item = 0; item < count; item++) {
    // Количество элементов в обном массиве. Vec4 vec3 Float Mat4

    let byteStrideItem = byteStride * item; // byteStride Сколько нужно байт отспупать между разными item

    for (let elemen = 0; elemen < elementOfType; elemen++) {
      // VEC4 4 злемента  VEC3  3 элемента Scalar 1 элемент  Считаем елемент типа данных  например в матрице Maт4 Это 16 элеентов

      let elemenBaty = elemen * T.BYTES_PER_ELEMENT; // Байт в конкртеном элементе // например в FLOAT 4 UINT 2

      for (let b = 0; b < lenBelement; b++) {
        // Количество байт в числе  2 или 4
        dView.setUint8(
          b,
          blob.charCodeAt(byteOffset + byteStrideItem + elemenBaty + b)
        );
      }
